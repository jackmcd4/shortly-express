...
exports.testPassword = function(password, user, cb) {
  var some = bcrypt.genSaltSync(10);
...
exports.checkUser = function (req, res, next) {
  ...
  if (req.headers.cookie && req.headers.cookie.slice(8)) {
    ...
    // new Session({'token': token}).fetch().then(function(found) {
    //   var expiration = found.get('expiration');
    //   console.log(expiration, new Date(expiration), new Date());

    //   if (expiration )

    //   //look at found expiration
    // });
    next();
  } else {
